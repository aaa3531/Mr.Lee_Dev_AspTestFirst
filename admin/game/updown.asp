...
<%
  ...
  ' game list 읽기
  strSQL = "p_sm_game_updown_kosdaq_list '" & today & "'"
...
<!-- #include virtual="/_include/guide_admin_game.inc" -->
    ...
    Do While Not rsDays.EOF  %> 
      ...
      <a href="updown.asp?today=<%=rsDays("yyyymmdd") %>">
      <span style="color:#ff6600;font-weight:bold;padding:2px;"><%=rsDays("gameday") %></span>
      </a>
      <% else %>
      <a href="updown.asp?yyyymm=<%=yyyymm %>&today=<%=rsDays("yyyymmdd") %>">
  ...
  <div style="padding:10px;text-align:center;background-color:#dddddd;font-weight:bold;">KOSDAQ UP&DOWN</div>
  <div style="margin:0 0 10px 0;padding:5px;text-align:center;line-height:180%;background-color:#ffffff;">
  <% if game_no > "0" then %>
    <form action="updown_update.asp" id="form3" name="formTool" method="post">
  ...
  <% else %>
    <form action="updown_insert.asp" id="form4" name="formTool" method="post">
  ...
  <% end if %>
    ...
    <table cellSpacing="0" cellPadding="0" border="1" ID="Table3" width="100%">
        ...
        <a href="updown.asp?game_no=<%=rsGame("game_no") %>"><%=rsGame("yyyymmdd") %>일 <%=rsGame("turn_no") %>회차</a>
        </div>
        </td>
        <td width="15%">
        <div style="padding:5px;text-align:center;"> 
