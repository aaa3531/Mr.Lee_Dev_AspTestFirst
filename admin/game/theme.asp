...
<!-- #include virtual="/_include/guide_admin_game.inc" -->
  ...
  <div style="margin:0 0 0 0;padding:10px;text-align:center;background-color:#dddddd; font-weight:bold;">테마</div>
  <div style="padding:5px; text-align:center; background-color:#ffffff;">
  
  <% if theme_no > "0" then %>
    <form action="theme_update.asp" id="formTool" name="formTool" method="post">
    ...
    <a href="theme.asp">[NEW]</a>&nbsp;
    <input id="submit1" name="submit1" type="submit" value="테마수정">
    </form>
  <% else %>
    <form action="theme_insert.asp" id="form1" name="formTool" method="post">
  ...
  <% end if %>
        ...
        <tr height="25" style="border-bottom:dotted 1px #888888;"> 
         <td width="30%" align="center" style="border-right:dotted 1px #ffffff;"><a href="theme.asp?page=<%=cPage%>&theme_no=<%=rsTheme("theme_no")%>"><%=rsTheme("theme_name") %></a></td>    
        ...
        <td width="20%"  align="center" style="border-right:dotted 1px #ffffff;">
        <a href="theme_delete.asp?page=<%=cPage%>&theme_no=<%=rsTheme("theme_no") %>"><span class="linkbtn">X</span></a>
        </td>
        </tr>
        <%                                
        	rsTheme.MoveNext
  ...
  <% if theme_name = "" then %>왼쪽에서 테마명을 선택하세요.<% else %><%=market_cd %> : <%=theme_name %><% end if %>
   ...
   <table cellpadding=0 cellspacing=0 border=0 width="100%">
        ...
        <% if FirstPage <> 1 then
          ...
          <td width="10%">
          <a href="theme_stock_set.asp?page=<%=cPage%>&theme_no=<%=theme_no %>&stock_no=<%=rsStockExist("stock_no") %>"><span class="linkbtn">-</span></a>
          </td>
          </tr>
  ...
  <form action="theme.asp" id="form2" name="formTool" method="post">
  ...
  <input id="submit3" name="submit1" type="submit" value="종목검색">  
    ...
    <table width="100%" cellpadding=0 cellspacing=0 border=0>
        ...
        %>
          ...
          <% if theme_no > 0 then %>
          <td width="10%">
          <a href="theme_stock_set.asp?page=<%=cPage%>&theme_no=<%=theme_no %>&stock_no=<%=rsStock("stock_no") %>"><span class="linkbtn">+</span></a>
          </td>
          <%else %>
          <td width="10%">
          </td>
    ...
    <%
	Response.Write ShowPageBar("theme.asp", Cus_pageSize, totalRecord, cPage, "/images/btn_board_pre.gif","/images/btn_board_next.gif",Cus_Tar)
    %>
					</td>
				</tr>
			</table>
