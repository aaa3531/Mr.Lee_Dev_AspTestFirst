...
<%
  ...
  if request("terms_no") = "" then	
    ...
    strSQL = "p_sm_private_info_detail '" & terms_no & "' "
    ...
    if NOT rs.EOF and NOT rs.BOF then
      ...
      default_flag = rs("default_flag")
    end if 
    set rs = nothing
  end if 
  
  ' terms 읽기
  strSQL = "p_sm_private_info_list "
...
<% membermenu = "PRIVATE_INFO"
   menu_desc = "개인정보 수집 및 이용안내"
%>
<!-- #include virtual="/_include/guide_admin_site.inc" -->
  ...
  <% if terms_no > "0" then %>
    <form action="private_info_update.asp" id="formTool" name="formTool" method="post">
    ...
    <td width="15%" align=center>사용
    <% if default_flag = "1" then %>
    <input type="checkbox" name="default_flag" style="width:20px;" class="input" ID="Text4" value="1" checked>
    <% else %>
    <input type="checkbox" name="default_flag" style="width:20px;" class="input" ID="Checkbox1" value="1">
    <% end if %>
    </td>
    <td width="15%" align="center">
    <a href="private_info.asp">[NEW]</a>
  ...
  <% else %>
    <form action="private_info_insert.asp" id="form1" name="formTool" method="post">
  ...
  <% end if %>
    ...
    <table width="100%" cellpadding=0 cellspacing=0 border=0>
        ...
        <% if FirstPage <> 1 then
          ...
          <a href="private_info.asp?page=<%=cPage%>&terms_no=<%=rsTerms("terms_no") %>"><%=rsTerms("terms_desc") %></a>
          </td>
          <td width="20%" align="center">
          <a href="private_info.asp?page=<%=cPage%>&terms_no=<%=rsTerms("terms_no") %>"><%=rsTerms("terms_version") %></a>
          </td>
          <td width="15%" align="center">
          <%=rsTerms("start_date") %>
          </td>
          <td width="15%" align="center">
          <%=left(rsTerms("update_date"),10) %>
          </td>
          <td width="20%" align="center">
          <a href="private_info_detail.asp?terms_no=<%=rsTerms("terms_no") %>"><span class="linkbtn">작성</span></a>
          &nbsp;<a href="private_info_delete.asp?page=<%=cPage%>&terms_no=<%=rsTerms("terms_no") %>"><span class="linkbtn">삭제</span></a>
          </td>
          </tr>
          <% else %>
        ...
        %>
		<% else %>
		<tr height="50" style="border-bottom:dotted 1px #dddddd;">
...
<%
	Response.Write ShowPageBar("private_info.asp", Cus_pageSize, totalRecord, cPage, "/images/btn_board_pre.gif","/images/btn_board_next.gif",Cus_Tar)
%>
					</td>
				</tr>
			</table>
