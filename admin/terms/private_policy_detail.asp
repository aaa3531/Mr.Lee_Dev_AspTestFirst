...
<%
  ...
  'detail 읽기
   if request("detail_no") = "" then	
    ...
    strSQL = "p_sm_private_policy_detail_detail '" & detail_no & "' "
  ...
  Else
   ...
   ' terms Detail 뿌리기
  strSQL = "p_sm_private_policy_detail_list '" & terms_no & "' "
...
<!-- #include virtual="/_include/guide_admin_site.inc" -->
  ...
  <% if terms_no > "0" then %>
    ...
    <a href="private_policy.asp"><input type="button" value="목록"></a>
    </td>
    </tr>  
    </table>
  <% else %>
  <% end if %>
    ...
    <% if detail_no > 0  then %>
    <form action="private_policy_detail_update.asp" id="form1" name="formTool" method="post">
    ...
    <a href="private_policy_detail.asp?terms_no=<%=terms_no%>">[NEW]</a> 
    </td>
    </table>
    ...
    <% else %>
    <form action="private_policy_detail_insert.asp" id="form2" name="formTool" method="post">
    ...
    <% end if %>
        ...
        <% Do While Not rsDetailList.EOF   %>
           <div style="padding:5px; background:#dddddd; font-size:14px; font-weight:bold;"><%=rsDetailList("order_seq") %>. &nbsp;<%=rsDetailList("section_desc") %>
            <a href="private_policy_detail_delete.asp?terms_no=<%=rsDetailList("terms_no") %>&detail_no=<%=rsDetailList("detail_no") %>"><span style="color:#ff6600; font-weight:bold;">X</span></a>
           </div>
     
           <div style="padding:50px;"><a href="private_policy_detail.asp?terms_no=<%=terms_no%>&detail_no=<%=rsDetailList("detail_no") %>"><%=rsDetailList("detail_desc") %></a></div>
        <%                           
