...
<%
  ...
  ' stock 읽기
  strSQL = "p_sm_stock_kospi_list '" & Request("key") & "'"
...
<!-- #include virtual="/_include/guide_admin_game.inc" -->
  ...
  <% else %>
    <form action="stock_kospi_insert.asp" id="form1" name="formTool" method="post">
  ...
  <% end if %>
    ...
    <table cellpadding=0 cellspacing=0 border=0>
          ...
          <td width="80" align="center"><a href="kospi.asp?key=CODE">종목코드</a></td>
          <td width="200" align="center"><a href="kospi.asp?key=NAME">종목명</a></td>
        ...
        <% if FirstPage <> 1 then
          ...
          <td width="80" align="center">
          <a href="kospi.asp?page=<%=cPage%>&key=<%=request("key") %>&stock_no=<%=rsStock("stock_no") %>"><%=rsStock("stock_cd") %></a>
          </td>
          <td width="200">
          <a href="kospi.asp?page=<%=cPage%>&key=<%=request("key") %>&stock_no=<%=rsStock("stock_no") %>">
          <input type="button" value="<%=rsStock("stock_name") %>" />
          </a>
          </td>
          <td width="200">
          <a href="kospi.asp?page=<%=cPage%>&key=<%=request("key") %>&stock_no=<%=rsStock("stock_no") %>"><%=rsStock("company_name") %></a>
          </td>
          ...
          <td width="97" align="center">
          <a href="stock_kospi_image_set.asp?page=<%=cPage%>&stock_no=<%=rsStock("stock_no") %>"><span class="linkbtn">SET</span></a>
          </td>
          </tr>
          <% else %>
          ...
          <td width="200">
          <a href="kospi.asp?page=<%=cPage%>&key=<%=request("key") %>&stock_no=<%=rsStock("stock_no") %>"><input type="button" value="<%=rsStock("stock_name") %>" /></a>
          </td>
          <td width="200">
          <a href="kospi.asp?page=<%=cPage%>&key=<%=request("key") %>&stock_no=<%=rsStock("stock_no") %>"><%=rsStock("company_name") %></a>
          </td>
...
<%
	Response.Write ShowPageBar("kospi.asp", Cus_pageSize, totalRecord, cPage, "/images/btn_board_pre.gif","/images/btn_board_next.gif",Cus_Tar)
%>
					</td>
				</tr>
			</table>
